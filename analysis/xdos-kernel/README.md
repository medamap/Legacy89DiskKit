...
- **Geometry Translation Constraints**: Whether the exact transform `(C * 2 + H, R)` used for 2D media spans other physical density metrics and translation engines down the layer. Boot/early-area observations are now cataloged at the raw level in `boot_and_io_notes.md`. Raw early-area sector spans for Track 0 and Track 1 are now cataloged at the observation level.
