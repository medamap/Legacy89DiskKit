...
class Program
{
    ...
    private static IFileCopyService _fileCopyService = null!;
    ...
    static Task Main(string[] args)
    {
        ...
        _fileCopyService = host.Services.GetRequiredService<IFileCopyService>();
    ...
    private static async Task CopyFile(string[] parameters)
    {
        if (parameters.Length < 3)
        {
            Console.WriteLine("Usage: copy <source-disk> <dest-disk> <filename> [--source-fs <type>] [--dest-fs <type>]");
            Console.WriteLine("Example: copy source.d88 dest.d88 GAME.BAS --source-fs hu-basic --dest-fs hu-basic");
            return;
        }

        var sourceDisk = parameters[0];
        var destDisk = parameters[1];
        var fileName = parameters[2];
        
        var sourceFsType = GetFileSystemParameter(parameters, "--source-fs");
        var destFsType = GetFileSystemParameter(parameters, "--dest-fs");

        if (sourceFsType == null || destFsType == null)
        {
            Console.WriteLine("Error: Both --source-fs and --dest-fs parameters are required");
            Console.WriteLine("Supported filesystems: hu-basic, fat12, n88-basic, msx-dos, cpm, cdos");
            return;
        }
        ...
        try
        {
            var result = await _fileCopyService.CopyFileAsync(
                ...
                destDisk, 
                fileName,
                new Legacy89DiskKit.DiskOperation.Domain.Model.FileCopyOptions
                {
                    ConflictResolution = Legacy89DiskKit.DiskOperation.Domain.Model.ConflictResolution.AutoRename,
                    Progress = new Progress<Legacy89DiskKit.DiskOperation.Domain.Model.FileCopyProgress>(p =>
                    {
                        Console.Write($"\rCopying {p.FileName}: {p.PercentComplete:F0}%");
                    })
                });
            ...
            if (result.Success)
            {
                Console.WriteLine($"Successfully copied: {result.SourceFileName} -> {result.DestinationFileName}");
                if (result.ConversionType != Legacy89DiskKit.DiskOperation.Domain.Model.FileNameConversionType.None)
                {
                    Console.WriteLine($"File name was {result.ConversionType.ToString().ToLower()}");
                }
                Console.WriteLine($"Bytes copied: {result.BytesCopied:N0}");
            }
            else
            {
                Console.WriteLine($"Failed to copy file: {result.ErrorMessage}");
            }
        }
        catch (Exception ex)
        {
    ...
    {
        using var shell = new InteractiveShell(_diskContainerFactory, _fileSystemFactory, _characterEncodingService, _fileCopyService);
        shell.Run();
    }
    ...
    private static string? GetFileSystemParameter(string[] parameters, string paramName)
    {
        for (int i = 0; i < parameters.Length - 1; i++)
        {
    ...
    private static void ShowHelp()
    {
        ...
        Console.WriteLine("  copy <source-disk> <dest-disk> <file>      Copy file between disk images");
        Console.WriteLine("       --source-fs <type> --dest-fs <type>");
        ...
        Console.WriteLine("  Legacy89DiskKit copy source.d88 dest.d88 GAME.BAS --source-fs hu-basic --dest-fs hu-basic");
    }
}