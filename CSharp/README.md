...
Legacy89DiskKit.CLI format disk.d88 --filesystem Cpm
```
...
| 機能 | Hu-BASIC | N88-BASIC | FAT12 | CP/M |
|------|----------|-----------|-------|------|
| ディスク作成・フォーマット | ✅ | ✅ | ✅ | ✅ |
| ファイル読み取り | ✅ | ✅ | ✅ | ✅ |
| ファイル書き込み | ✅ | ✅ | ✅ | ✅ |
...
- **CP/M汎用**: ASCII（7ビット）
- **CP/M PC-8801**: PC-8801固有文字対応
- **CP/M X1**: X1固有文字対応
- **CP/M MSX-DOS**: Shift-JIS（MSX-DOS互換）

## 制限事項

- ディレクトリはサポートされていません（ルートディレクトリのみ）
- マルチバイト文字（漢字等）はサポートされていません（MSX-DOSを除く）
- DSK形式での2HD以外のサポートは制限があります

### CP/Mファイルシステムに関する重要な注意事項

CP/Mは128バイト（1レコード）単位でファイルを管理するため、以下の制限があります：

#### ファイルサイズとEOFマーカー
- **128バイト境界に揃わないファイル**: CP/M標準に従い、ファイル末尾にEOFマーカー（Ctrl+Z = 0x1A）が自動的に追加されます
- **インポート時**: 元のファイルサイズが128の倍数でない場合、最後のレコードにEOFマーカーが追加されます
- **エクスポート時**: EOFマーカーを検出してファイルサイズを決定します

#### 現代的な使用における考慮事項
- **バイナリファイル**: 正確なバイト単位のサイズ保持が必要な場合は、ZIPなどでアーカイブしてから転送することを推奨します
- **大容量ファイル**: 16KB以上のファイルはマルチエクステント（複数のディレクトリエントリ）を使用します
- **テキストファイル**: EOFマーカーにより正確に復元されます

#### 互換性
- 本実装はCP/M 2.2標準に準拠しており、実機のCP/Mシステムとの完全な互換性を保証します
- 他のCP/Mエミュレータやツールとのファイル交換も可能です
