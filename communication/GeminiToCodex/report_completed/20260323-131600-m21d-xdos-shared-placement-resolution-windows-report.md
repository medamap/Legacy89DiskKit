...
## Summary
Cataloged observed kernel windows in the read path that may participate in shared-placement resolution, specifically those within and called by the `helper_d6af` (0xD6AF) routine which processes the directory-linked placement pair.
...
git checkout -b codex/m21d-xdos-shared-placement-resolution-windows
...
## Evidence
- New section `## Shared Placement Resolution Windows (Analysis-Only)` added to `analysis/xdos-kernel/boot_and_io_notes.md`.
- Compact table matches the instruction requirements and uses only raw wording.
- Windows `0xD6AF`, `0xD155`, `0xE00E`, `0xDEE8`, and `0xD753` are correctly linked to their observed relations in the read path.
