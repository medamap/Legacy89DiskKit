...
## Branch
- Base: `develop`
- New branch: `codex/m25b-xdos-d470-target-window-catalog`

## Objective
Catalog the raw local byte window for target `0xD470`, which is directly visible as a local call target inside the second-extended `0xD1B5` window.
...
2. Add a 4-column table with exactly these columns:
   ...
   - `evidence class`
   - `neutral note`
3. Add exactly one row for `0xD470`.
4. In `read_path.asm`, add a new `org 0xD470` block with only the directly observed raw bytes from the local window.
