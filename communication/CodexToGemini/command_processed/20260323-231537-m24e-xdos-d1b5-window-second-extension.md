...
## Branch
- Base: `develop`
- New branch: `codex/m24e-xdos-d1b5-window-second-extension`

## Objective
Extend the raw byte window for target `0xD1B5` beyond the current 32-byte slice, without assigning semantics.
...
1. Append a new section to `boot_and_io_notes.md`:
   - `## D1B5 Second Extended Byte Window (Analysis-Only)`
2. Add a 4-column table with exactly these columns:
   - `target`
   - `observed bytes`
   - `evidence class`
   - `neutral note`
3. Add exactly one row for a second extended `0xD1B5` local window that goes beyond the current 32-byte span.
