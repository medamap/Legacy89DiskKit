...
## Branch
- Base: `develop`
- New branch: `codex/m24c-xdos-d1b5-extended-literal-catalog`

## Objective
Catalog only directly observed literals or immediate values inside the current extended `0xD1B5` window.
...
2. Add a 4-column table with exactly these columns:
   ...
   - `evidence class`
   - `neutral note`
3. Add rows only for directly visible literals/immediates in the currently cataloged extended `0xD1B5` window, including any new values beyond the initial `0x0328` and `0x0522`.
