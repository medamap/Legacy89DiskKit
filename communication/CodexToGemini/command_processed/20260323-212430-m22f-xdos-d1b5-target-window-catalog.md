...
## Branch
- Base: `develop`
- New branch: `codex/m22f-xdos-d1b5-target-window-catalog`

## Objective
Catalog the raw local byte window for target `0xD1B5`, which is directly visible as a local call target inside the already-cataloged `0xD753` window.
...
- Preserve existing headings, bullets, tables, and comments.
...
2. Add a 4-column table with exactly these columns:
   ...
   - `evidence class`
   - `neutral note`
3. Add exactly one row for `0xD1B5`.
4. In `read_path.asm`, add a new `org 0xD1B5` block with only the directly observed raw bytes from the local window.
