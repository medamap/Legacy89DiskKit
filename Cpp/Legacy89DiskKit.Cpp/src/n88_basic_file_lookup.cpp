...
{
std::optional<N88BasicFileEntry> N88BasicFileLookup::FindByName(
...
{
    const auto files = N88BasicDirectoryListing::ListFiles(directory_sectors, fat_data, config);
...
bool N88BasicFileLookup::Exists(
    const std::vector<std::vector<std::uint8_t>>& directory_sectors,
    const std::vector<std::uint8_t>& fat_data,
    const N88BasicConfiguration& config,
    const char* file_name)
